...
def build_atlas_projection(
    ...
    """Aggregate the semantic graph at Field -> Concept -> ConceptState scale.

    This is a reader projection, not canonical evidence. It groups attested concept
    states under the fields declared on their diachronic Concept identities and
    attaches nearby typed semantic nodes without inventing historical transitions.
    """
    ...
    states = {s['id']: s for s in concept_states}
    claims = normalize_assertions(entities, questions, concept_states, assertions)
    typed = [
        c for c in claims
        if c.get('publishable')
        and c.get('default_network_visible')
        and c.get('semantic_layer') in {'historical', 'mathematical'}
    ]
    ...
    for concept_id, concept in concepts.items():
        ...
        for state in sorted(states_by_concept.get(concept_id, []), key=lambda s: (_year(s) is None, _year(s) or 0, s['id'])):
            ...
            concept_state_rows.append({
                'id': state['id'],
                'year': _year(state),
                'period': state.get('period', {}),
                'label': state.get('label', ''),
                'related_node_ids': neighbors,
            })
        ...
        years = [row['year'] for row in concept_state_rows if row['year'] is not None]
        concept_rows.append({
            'id': concept_id,
            'name': concept.get('name', concept_id),
            'fields': concept.get('fields', []),
            'state_count': len(concept_state_rows),
            'first_attested_state_year': min(years) if years else None,
            'last_attested_state_year': max(years) if years else None,
            'states': concept_state_rows,
            'related': dict(related_by_kind),
        })

    field_rows = []
    for field in fields:
        field_id = field['id']
        members = [row for row in concept_rows if field_id in row.get('fields', [])]
        members.sort(key=lambda row: (
            row['first_attested_state_year'] is None,
            row['first_attested_state_year'] or 0,
            row['name'],
        ))
        ...
        field_rows.append({
            **field,
            'concept_ids': [row['id'] for row in members],
            'concept_count': len(members),
            'concept_state_ids': state_ids,
            'concept_state_count': len(state_ids),
            'related_node_ids': related_ids,
        })

    cross_field_concepts = [
        {'concept_id': row['id'], 'fields': row['fields']}
        for row in concept_rows if len(row.get('fields', [])) > 1
    ]
    ...
    return {
        ...
        'cross_field_concepts': cross_field_concepts,
        'notes': [
